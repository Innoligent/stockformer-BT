...
class DRLAgent:
    ...
    def train_model(self, model, tb_log_name, check_freq, ck_dir, log_dir, eval_env, total_timesteps=5000, verbose=1, deterministic=True):
        ...
        print(f"[Start RL Training] timesteps before training: {model.num_timesteps}")
        model = model.learn(
            total_timesteps=total_timesteps,
            tb_log_name=tb_log_name,
            callback = callback
        )
        print(f"[End RL Training] timesteps after training: {model.num_timesteps}")
    ...
    def DRL_prediction_load_from_file(model_name, environment, cwd, deterministic=True):
        ...
        while not done:
            ...
            #print(action)
            if 'action_log' not in locals():
                action_log = []

            # 在每步中：           
            codes = environment.data.tic.values if hasattr(environment.data, 'tic') else [f'stock_{j}' for j in range(len(action))]

            record = {}
            for j, code in enumerate(codes):
                record[code] = action[j]
        ...
        df_actions.to_csv('results/original_actions_float.csv', index=False)
        print("已保存原始动作到 results/original_actions_float.csv")
