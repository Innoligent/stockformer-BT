    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x1, batch_x2, batch_y) in enumerate(train_loader):
                # 检查 batch 中每个样本的时间长度
                for j in range(batch_x1.shape[0]):
                    if batch_x1[j].shape[1] == 0 or batch_x2[j].shape[1] == 0:
                        print(f"Batch {i}, Sample {j} has empty time dimension! x1: {batch_x1[j].shape}, x2: {batch_x2[j].shape}")
            '''
