...
class Exp_mae(Exp_Basic):
    ...
    def train(self, setting):
        ...
        '''        
        # === 打印模型结构和是否包含 tokenConv/position_embedding ===
        print("\n================ 模型结构摘要 ====================")
        print(self.model)

        print("\n================ 模型参数中是否包含 tokenConv / position_embedding ====================")
        found = False
        for name in self.model.state_dict().keys():
            if 'tokenConv' in name or 'position_embedding' in name:
                print("yes,", name)
                found = True
        if not found:
            print("没有找到 tokenConv 或 position_embedding 相关参数，模型结构可能不正确或未激活这些层")
        print("=====================================================\n")
        '''
